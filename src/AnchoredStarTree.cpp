...
////////////////////////////////////////////////////////////////////////////////
// AnchoredStarTree R functions

namespace {

std::vector<std::string> anchored_character_vector_to_strings(CharacterVector x) {
...
void validate_anchored_star_sequence(const std::string& seq) {
  if(seq.empty()) {
...
void validate_anchored_star_sequences(const std::vector<std::string>& sequences,
                                      const char* label) {
  if(sequences.empty()) {
  ...
  for(const std::string& seq : sequences) {
    validate_anchored_star_sequence(seq);
  }
}

void validate_anchored_star_params(const int max_distance,
...
SEXP anchored_string_to_charsxp(const std::string& x) {
  return Rf_mkCharLen(x.data(), checked_r_len(x.size()));
}

DataFrame anchored_startree_empty_match_result() {
...
DataFrame anchored_startree_pairs_to_dataframe(
    const std::vector<startree::PairRecord>& pairs,
    const startree::AnchoredInputData& target_data,
    const startree::AnchoredInputData& query_data) {
  if(pairs.empty()) {
    return anchored_startree_empty_match_result();
  }
  ...
  for(size_t i = 0; i < n; ++i) {
    ...
    SET_STRING_ELT(query, checked_r_xlen(i),
                   anchored_string_to_charsxp(query_data.seqs[pair.query_index].seq));
    SET_STRING_ELT(target, checked_r_xlen(i),
                   anchored_string_to_charsxp(target_data.seqs[pair.target_index].seq));
...
std::vector<startree::PairRecord> flatten_anchored_pair_chunks(
...
std::vector<startree::PairRecord> run_anchored_lcp_search_impl(
  ...
  return flatten_anchored_pair_chunks(per_block);
}
...
AnchoredStarTreeRXPtr AnchoredStarTree_create(CharacterVector sequences,
  ...
  auto raw_sequences = anchored_character_vector_to_strings(sequences);
  validate_anchored_star_sequences(raw_sequences, "sequences");
  validate_anchored_star_params(max_distance, mismatch_cost, gap_cost);
...
DataFrame AnchoredStarTree_self_search(CharacterVector sequences,
  ...
  return anchored_startree_pairs_to_dataframe(pairs, data, data);
}
...
CharacterVector AnchoredStarTree_to_vector(AnchoredStarTreeRXPtr xp) {
  ...
  for(size_t i = 0; i < xp->data.seqs.size(); ++i) {
    SET_STRING_ELT(out, checked_r_xlen(i),
                   anchored_string_to_charsxp(xp->data.seqs[i].seq));
  }
  return out;
}
...
DataFrame AnchoredStarTree_result(AnchoredStarTreeRXPtr xp) {
  return anchored_startree_pairs_to_dataframe(xp->self_pairs, xp->data, xp->data);
}
...
DataFrame AnchoredStarTree_search(AnchoredStarTreeRXPtr xp,
  ...
  auto raw_query = anchored_character_vector_to_strings(query);
  validate_anchored_star_sequences(raw_query, "query");
  const startree::AnchoredInputData query_data =
  ...
  return anchored_startree_pairs_to_dataframe(pairs, xp->data, query_data);
}
