...
- **StarTree** (`star_tree()`): a specialized DNA similarity join class. It is a
  ...
  being a direct reimplementation. For small edit-distance DNA self-joins it
  typically outperforms the other two classes (see the benchmark below).
...
StarTree only supports global/Levenshtein-style DNA alignment with `A`, `C`,
`G`, `T`, and `N`. It does not support custom substitution matrices, affine
gaps, Hamming search, anchored search, or per-query distance thresholds.
