...
# encode_source("~/GoogleDrive/treedist/tests/original_implementation.cpp")
# ogsource <- c(
...
run_dnatree <- function(query, target, max_distance=NULL, max_fraction=NULL) {
  x <- DNATree$new()
  x$insert(target)
  x$search(query, max_distance = max_distance, max_fraction = max_fraction, mode = "levenshtein", display_progress=F)
}
...
run_stringdist <- function(query, target, max_distance=NULL, max_fraction=NULL) {
  results <- stringdist::stringdistmatrix(query, target, method = "lv", nthread=1)
}
...
# nseqs <- c(10,30,100,300,1000,3000,10000,30000)
nseqs <- c(30,100,300,1000,3000)
...
ggplot(grid, aes(x = nseqs, y = time, color = method)) + geom_point() + geom_smooth() +
  scale_x_log10() + 
  scale_y_log10() + 
